...
<h3 id="간단한-produce-함수-구현" tabindex="-1"><a class="header-anchor" href="#간단한-produce-함수-구현" aria-hidden="true">#</a> 간단한 produce 함수 구현</h3>
<p>produce는 이런 형태로 구현가능하다. 상태를 clone하고 명령 함수를 통해 상태를 변경하는 것이다.</p>
...
<span class="token punctuation">}</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>테스트해보면 이런 결과가 나온다. immer를 사용했을 때와 다른 점은 <code v-pre>nextState[0] === baseState[0]</code>이 부분이다. 변경하지 않는 부분도 새로 만든 점이 다르다.</p>
...
<p>해당 함수를 구현해서 서비스에 사용중이다. 만약에 영속적 자료 구조의 이점이 필요없고, 단순히 immer의 인터페이스때문에 사용중이라면 해당 기능으로 대체 가능하다.</p>
<p>clone 함수는 JSON에서 지원하는 타입만 사용가능하다. 다양한 타입이나 좀더 좋은 성능의 clone함수는 <RouterLink to="/essay/clone-function.html">경우에 따른 clone 함수</RouterLink>에서 다룬다.</p>
</div></template>
