...
지연 평가 언어가 아니여도 지연 평가를 구현해서 사용하기도 한다. 지연 시퀀스, 스트림, 게으른 컬렉션으로 도서마다 다양하게 불리는 자료구조이다. 여기서는 지연 리스트로 부르겠다. 지연 리스트는 언어에서 `lazy`라는 키워드를 통해 지원하는 언어도 있지만 없는 언어는 `Iterator`를 통해서 구현한다. 자바스크립트의 지연 리스트는 [지연 리스트](/fe-dev/paradigm/functional2.md#지연-리스트) 본문에서 다루겠다.
